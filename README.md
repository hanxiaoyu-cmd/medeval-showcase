...
![version](https://img.shields.io/badge/VERSION-v1.36v-6c8cff?style=for-the-badge&labelColor=0b0f1a)
...
![金字塔模式:3D 玻璃任务金字塔与 COPILOT 编排](assets/gx-pyramid.png)

</div>

---

## ✦ v1.36v 更新 ── 完整双语 · 检索按平台锚定 · 事后补全代理 `i18n · FDA/EU/NMPA 锚定 · 覆盖审查`

- **🌐 全界面中英双语** — 14 个前端文件、逾千处界面文案完成 i18n;含知识图谱、检索诊断、金字塔对话等所有角落。顶部一键切换,`?lang=en` 深链直达。历史任务名等用户数据保持原文。
- **🎯 检索按申报平台锚定** — 官方法规检索不再无约束发散到无关辖区:按用户选择的平台锚定主监管机构(FDA / EU MDR / NMPA…),别国先例仅作次要参考且不计为覆盖缺口;ISO/ASTM/IEC 国际标准始终在范围内。注入到主检索计划 + 策略反思 + 递归三处。
- **🔎 检索后补全代理(可选)** — 固定检索全部跑完后,由 agent 审查覆盖是否足以支撑评分,并发现**预设泳道结构性漏掉的器械特异角度**(如可降解镁螺钉的降解速率 / 氢气释放 / 儿童适应症),生成补充查询补搜。只增不改、失败静默跳过、补充证据同样过相关性过滤 + 防火墙 + 归档 —— 降级安全。
- **⚡ 任务详情加载 149s → 5.5s** — 历史任务打开由事件循环阻塞的 async 改线程池同步(一个慢请求不再冻结全站),并补上缺失的数据库索引(全表扫描 → 索引查找)。
- **🖇 深链与可分享** — `?view=workbench`(直入工作台)、`?task=<id>`(直达某任务结果)、`?theme`、`?lang`,便于演示、截图与分享。

<div align="center">

`v1.36v = 完整双语界面 · 检索锚定目标市场 · 事后覆盖补全 · 任务详情 27× 提速`
...
MedEval 有两个互通入口:**金字塔模式**(一句话编排、看着任务建成)与**经典工作台**(逐阶段精细控制)。两者共享同一批任务、会话与结果,随时一键互切。

### ① 打开 · 选择模式

打开应用即进入 3D 金字塔场景。周围是已完成任务凝固成的玻璃金字塔(**绿 = 器械评估,蓝 = 文档生成**),右下角是 MEDEVAL COPILOT 编排助手。

![金字塔模式:任务金字塔与 COPILOT 编排](assets/gx-pyramid.png)

- **🔺 金字塔模式** — 把材料拖给 COPILOT,打一句"评估这个器械"或"生成一份 CEP",它自动上传 → 清洗 → 发起;运行中的任务化作场景中心的金字塔,**随流水线阶段逐层建成**;点任意金字塔即恢复它的会话与结果。
- **🖥 经典工作台** — 点"经典工作台 →"进入分步界面(新手推荐,每步都看得见、可检查)。

![经典工作台:悬浮玻璃侧栏 + 五步流程 + 逐阶段模型](assets/gx-workbench.png)

顶部是**交付健康**总览(存储 / 配额 / 版本 / 各阶段模型),下方是分步流程;右侧可切换**评估 / 生成**模式、亮暗主题、中英文;左侧是可自动收纳的历史任务玻璃侧栏。

### ② 器械评估 · 五步

选「评估模式」,按五步走,全程约 20–30 分钟,发起后可离开、回来看结果:

| 步骤 | 系统做什么 | 你要做什么 |
|:--|:--|:--|
| **1 · 资料上传** | 接收多格式文件(PDF / Word / Excel / PPT / 图片),扫描件自动 OCR | 拖入说明书、临床资料、测试报告、风险管理、竞品资料;选申报平台(FDA 510(k)/PMA、BDD、EU MDR、NMPA) |
| **2 · 智能整理** | 并行清洗、去重、按内容识别每份材料,提炼出器械档案 | 等它跑完(不用手动分类) |
| **3 · 信息确认** | 展示提取的器械档案(名称 / 适应症 / 关键特征 / 分类) | 核对,确认无误或补充修正后继续 |
| **4 · 评估** | 联网取证——竞品 / 官方法规 / 标准 / 临床 / 安全,**按目标申报平台锚定检索**,反思覆盖缺口迭代补搜,再按监管维度逐项打分 | 等待(最耗时,十几分钟) |
| **5 · 结果解读** | 汇总总分、各维度、竞品对比、证据缺口,导出带完整引用的报告 | 查看、下载 DOCX / Markdown |

### ③ 读懂评估结果

![评估结果:评分环 · 维度分 · 最终报告门禁 · 行动建议](assets/gx-result.png)

- **评分环 + 维度分** — 总分与各监管维度得分;每项标注"大模型评的"还是"脚本兜底"。
- **竞品对比** — 自动从检索与上传材料中发现同类器械(示例:53 候选 → 裁决出具名竞品),按"事实 → 机理 → 审评后果"讲清实质差异,非参数罗列。
- **最终报告门禁** — 检索完成 / 全文归档 / 竞品裁决 / 引用接地 / 跨维度一致性逐项校验;材料不足以支撑结论时**诚实拦截**(`ungrounded_document`)并提示补什么料,绝不硬凑满分。
- **可追溯引用** — 每条结论都能点回原文件、页码、结构块。
- **行动建议** — 按优先级列出补强方向,直接对应低分维度。

### ④ 申报文档生成

顶部切「生成模式」:上传材料 → 系统标准化整理 → 选文档类型 → 按章节自动起草(引用绑定原始材料) → 下载 Word / Markdown。可选文档类型:

`CEP 临床评估计划` · `CER 临床评估报告` · `CIP 临床试验方案` · `IB 研究者手册` · `PMCF 计划` · `PMCF 研究概要`

> CEP / CER / CIP 打磨最成熟——均为 **24 个规范章节 + 章节关系图谱 + 专属正则检查表**(MDCG 2020-13 / BSI top-10 / ISO 14155:2020),终审对缺失项给出审评提示。

### ⑤ 玻璃质感界面 · 双语 · 双主题

全界面统一钢蓝玻璃质感:背景折射、白色棱边反射随指针流动、窗口随鼠标位置重心偏移的 3D 悬浮物理;历史任务侧栏悬浮玻璃、鼠标离开自动收纳、贴屏幕左缘再唤出。**完整中英双语**、**亮 / 暗双主题**,均可用 URL 深链直达(`?lang=en` / `?theme=dark` / `?view=workbench`)。

<table>
<tr>
<td width="50%"><img src="assets/gx-english.png" alt="英文界面"/><br/><em align="center">完整中英双语 · <code>?lang=en</code></em></td>
<td width="50%"><img src="assets/gx-workbench-dark.png" alt="暗色主题"/><br/><em align="center">亮 / 暗双主题 · <code>?theme=dark</code></em></td>
</tr>
</table>
